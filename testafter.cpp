...
// Multiplication 
void multiply(int arr[],int arrr[],int s_of_1n){
  int sum=0;
  int z=1,a;
  int osum[s_of_1n];
   for ( int i =-((2*s_of_1n)-4); i<s_of_1n; i++)
  {   
      osum[i]=0;   
  }
  ...
  { 
      int k=s_of_1n-1;
      a=arrr[i];
      for (int ii =i; ii>=-((2*s_of_1n)-4); ii--)
      {   
          if((a*arr[k])>9)
          {
             int j= (a*arr[k]) % 10;
             ...
             int jjj=jj/10;
             osum[ii]=osum[ii]+j;
             osum[ii-1]=osum[ii-1]+jjj;
          }
          else
          {
              
          osum[ii]=osum[ii]+(a*arr[k]);
